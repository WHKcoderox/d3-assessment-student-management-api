...
export class TeacherService {
  ...
  async studentsRegisteredToTeachers(
    ...
    try {
      ...
      await Promise.all(
        teacherEmails.map(async (teacher, idx) => {
          const teacherExists = await queryRunner.manager
            .getRepository(Teacher)
            .exists({ where: { email: teacher } });
      ...
      throw new BadRequestException({
        message:
          'Failed to query students registered under teachers. Check that the email(s) were spelled correctly and the database is configured correctly.',
      });
    } finally {
